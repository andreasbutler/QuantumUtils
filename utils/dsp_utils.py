...
def modulate_signal(sig, dt, freq, phase):
  ...
  point multiplication with signal to return the modulated signal.
...
def upconvert_and_filter(sig, upconversion_factor, filt=None):
  ...
  valid 'digital' signal, with, for example, the sampling rate of a realistic
  signal generator, and upconverting it to an artificially high resolution after
  which a filter can be applied that approximately reproduces the actual analog
  signal we expect a signal generator to output.
...
def get_times(signal, sampling_rate, offset=0):
    number_of_samples = len(signal)
    start_time = offset
    end_time = offset + number_of_samples / sampling_rate
    times = np.linspace(start_time, end_time, number_of_samples)
    return times
